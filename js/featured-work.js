document.addEventListener('DOMContentLoaded', () => {
    ...
    featuredWorks.forEach((item, index) => {
        // Remove existing anchor tag click behavior
        const innerWrapper = item.querySelector('.feature-inner-wrapper');
        if (innerWrapper) {
            const originalHref = innerWrapper.getAttribute('href');
            // Extract work ID from href, handling both /project#id and #id formats
            const workId = originalHref.includes('#') ? originalHref.split('#')[1] : '';
            console.log('Found work ID:', workId); // Debug log
            
            // Add click event to the outer wrapper as well
            ...
            // Function to handle work click
            function handleWorkClick(workId) {
                // L'imperatrice - Vertical Vimeo popup
                if (workId === 'limperatrice') {
                    console.log('Opening Vertical Vimeo modal for L\'imperatrice');
                    
                    const vimeoId = vimeoIds[workId] || '824636539';
                ...
                // Bob Moses - Instagram popup (or fallback to YouTube)
                    ...
                    if (instagramUrls[workId]) {
                        console.log('Opening Instagram modal for Bob Moses');
                        
                        // Instagram embed code
                        const instagramEmbed = `
                            <div class="instagram-post" style="display: flex; justify-content: center; align-items: center; width: 100%; height: 100%;">
                                <blockquote class="instagram-media" data-instgrm-permalink="${instagramUrls[workId]}" 
                                ...
                                <div style="padding:16px;"> <a href="${instagramUrls[workId]}" style="background:#FFFFFF; line-height:0; padding:0 0; text-align:center; text-decoration:none; width:100%;" target="_blank">
                        ...
                        // Ensure the Instagram post is centered
                        setTimeout(() => {
                            // Target both potential Instagram embed elements
                            const instagramMedia = document.querySelector('.instagram-container .instagram-media');
                            if (instagramMedia) {
                                instagramMedia.style.margin = '0 auto';
                                instagramMedia.style.display = 'block';
                            }
                            
                            // Also target .instagram-post for consistency with works.js
                ...
                // Default to YouTube for any other works
                else {
            ...
            // Add click event to the inner wrapper
            innerWrapper.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                handleWorkClick(workId);
            });
